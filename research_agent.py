    ...
    if st.button("🚀 実データ検索 ＆ サマリー生成を開始"):
        if not api_key:
            ...
            with st.spinner(f"ステップ1/2: 【{engine_choice}】から実在する論文を検索中..."):
                try:
                    # ここが横に長すぎたので改行して整理しました
                    if "Semantic Scholar" in engine_choice: 
                        papers = fetch_semantic_scholar(research_theme, paper_count)
                    elif "CiNii" in engine_choice: 
                        papers = fetch_cinii(research_theme, paper_count)
                    elif "PubMed" in engine_choice: 
                        papers = fetch_pubmed(research_theme, paper_count)
                    elif "arXiv" in engine_choice: 
                        papers = fetch_arxiv(research_theme, paper_count)
                    elif "OpenAlex" in engine_choice: 
                        papers = fetch_openalex(research_theme, paper_count)
            ...
            if not papers:
                ...
                with st.spinner(f"ステップ2/2: 見つかった {len(papers)} 件の論文データをAIに読み込ませ、統合サマリーを執筆中..."):
                    ...
                    try:
                        ...
                        # --- エクスポート機能 ---
                        st.markdown("---")
                        st.subheader("💾 レポートの保存・コピー")
                        
                        col1, col2 = st.columns(2)
                        
                        with col1:
                            st.download_button(
                                label="📥 Markdown形式（.md）で保存",
                                data=response.text,
                                file_name=f"ResearchReport_{research_theme}.md",
                                mime="text/markdown"
                            )
                        ...
                        with col2:
                            st.download_button(
                                ...
                                data=response.text,
                                file_name=f"ResearchReport_{research_theme}.txt",
                                mime="text/plain"
                            )
                            
                        with st.expander("📋 テキストを全選択して一括コピーする"):
                            st.text_area("枠内をクリックして「Ctrl+A (MacはCmd+A)」→「コピー」してください", response.text, height=200)
